...
export namespace CsvToDeepJson {
    ...
    function build(csvStringArray: string[]): any[] {
        const result = [];
        ...
        for (let i = 1; i < csvStringArray.length; i++) {
            ...
            for (let j = 0; j < currentRow.length; j++) {
                ...
                setDeep(
                    newObject,
                    currentCellInfo.path,
                    currentCellInfo.type === PrimitiveType.BOOLEAN ? parseBoolean(currentRawValue)
    ...
    function setDeep(baseObject: any, path: string, val: boolean | number | string | string[]) {
    ...
    function parseArray(str: string, separator: string): string[] {
        if (!str) return null;
    ...
    function toPathAndType(raw: string): string[] {
    ...
    enum PrimitiveType {
        ...
        NUMBER,
        BOOLEAN,
        STRING_ARRAY // TODO handle ARRAY'' of any PrimitiveType
    }
}
